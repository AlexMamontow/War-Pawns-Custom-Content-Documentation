# Unit Traits Reference

This file lists the built-in trait IDs available to custom scenarios and campaigns.

Use these IDs in JSON payload fields such as `traitIds`, `addTraitIds`, `removeTraitIds`, and `availableTraitIds`.

`traitId` is the element index in `TraitsConfig.Items`.

## Traits

| traitId | assetName | traitClass |
| --- | --- | --- |
...
| 4 | Hedehogs | Passive Regular Bonus Trait |
...
- `0 / NoneTrait` is the empty/no-trait entry.
- Prefer adding/removing traits through `modify_unit_traits` or `update_units` selector payloads.
- Trait behavior depends on the in-game trait implementation. This reference is an ID lookup table, not a full balance manual.
